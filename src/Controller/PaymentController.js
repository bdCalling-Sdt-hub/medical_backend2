...
const SavePayment = async (req, res) => {
    try {
        const newPayment = new PaymentModel({ ...req.body, doctor_amount: req.body?.amount - req.body?.amount * 30 / 100, });
...
const TransferBallance = async (req, res) => {
    try {
        ...
        // stripeAccount: existingStripeAccount?.accountInformation?.stripeAccountId,
        // }
        const amount = GetDoctorPayment?.services?.reduce((acc, service) => acc + service.price, 0) || 0;
        ...
        if (transfer.id) {
            ...
            const [result, appointment, savePayment, createNotification] = await Promise.all([
                PaymentModel.updateMany({ doctorId: doctorId, payment_doctor: false }, { $set: { payment_doctor: true } }),
                Appointment.updateOne({ _id: appointmentId }, { $set: { doctor_payment: true } }),
                DoctorPayment.save(),
                CreateNotification({ userId: GetDoctorPayment?.userId, doctorId, appointmentId: appointmentId, message: 'Received Payment', body: `You Have received $${totalPayable}`, type: 'payment' }, req.user)
            ])
...
const deleteStripeAccount = async (req, res) => {
    try {
        ...
        const { id } = req?.user;
        const result = await StripeAccountModel.findOne({ doctorId: id, _id: accountId });

        if (result) {
            await result.delete();
            return res.status(200).send({ success: true, message: "Stripe account deleted successfully" });
        } else {
...
module.exports = { Payment, SavePayment, createConnectedAccount, TransferBallance, UserGetPaymentHistory, GetDoctorPaymentHistory, GetAvailablePayment, GetMyCard, TransitionHistoryOverview, deleteStripeAccount }
